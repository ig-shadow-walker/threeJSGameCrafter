...
- Public Three.js skills are consolidated around the director plus specialist systems: concept/design, gameplay, AAA graphics, UI, debug/profile, QA/release, 3D generation, image generation, and audio generation.
- For from-scratch, vague, or "help me design a game" requests, start with `threejs-game-concept` to ideate, scope, and write a Game Design Document (`docs/game-design.md`) before building. Its core loop seeds the playable contract and its asset map pre-fills the sourcing ledger. Skip it for narrow fixes.
...
Use the scaffold's `npm run verify:visual` and `npm run inspect:canvas` when available, or `skills/threejs-qa-release/scripts/inspect-threejs-canvas.mjs` from this repo.
